...
function cacheElements() {
  ...
  els.siteFavicon = document.getElementById("siteFavicon"); // may be null if removed from HTML
...
function performSearch(query) {
  ...
  chrome.storage.local.get(null, items => {
    ...
    matches.slice(0, 30).forEach(match => {
      ...
      `;
      if (tags) {
        html += `<div class="ws-result-tags">${escapeHtml(tags)}</div>`;
      }
...
function exportNotesAsText() {
  chrome.storage.local.get(null, items => {
    ...
    a.href = url;
    a.download = "WebScribe_export.txt";
...
function setHeaderForUrl(url) {
  ...
  // Built-in Chrome favicon endpoint (only if favicon element exists)
  if (els.siteFavicon) {
...
function updateLastSaved(updatedAt) {
  ...
  els.saveStatus.textContent = `Last saved: ${time}`;
}
...
function setView(view) {
  ...
  els.viewEditor.classList.toggle("ws-view--active", view === "editor");
  els.viewAllNotes.classList.toggle("ws-view--active", view === "all");
  els.viewHistory.classList.toggle("ws-view--active", view === "history");
  els.viewSettings.classList.toggle("ws-view--active", view === "settings");

  // Update tab button styles
  const tabMap = {
    editor: els.tabEditorBtn,
    all: els.tabAllBtn,
    history: els.tabHistoryBtn,
  };

  Object.entries(tabMap).forEach(([key, btn]) => {
    btn.classList.toggle("ws-tab--active", view === key);
  });

  // Settings isn't a tab — deactivate all tabs when settings is shown
  if (view === "settings") {
    Object.values(tabMap).forEach(btn => btn.classList.remove("ws-tab--active"));
  }
